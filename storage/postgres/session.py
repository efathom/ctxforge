...
Provides persistent session storage with JSONB, optimistic locking, and full ACID support.
"""
...
@registry.register_session_store("postgres")
class PostgresSessionStore(ISessionStore):
    ...
    def _serialize_session(self, session: Session) -> Dict[str, Any]:
        """Serialize session to dict for JSONB storage."""
        return json.loads(session.model_dump_json())
    
    def _deserialize_session(self, row: Any) -> Session:
    ...
    async def load(self, session_id: str, user_id: str) -> Session:
        ...
        row = await self._manager.fetchrow(query, session_id)
    ...
    async def save(self, session: Session) -> None:
        ...
        upsert_query = f"""
            ...
            WHERE {self.config.sessions_table}.version < EXCLUDED.version
        """
    ...
    async def list_sessions(
        ...
        rows = await self._manager.fetch(query, user_id, limit, offset)
